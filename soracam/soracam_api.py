...
import soracam
import soracam.soracam_api_config as cfg
...
_MAX_API_RETRIES = os.environ.get('MAX_RETRIES', cfg.MAX_API_RETRIES)
_SORACOM_ENDPOINT = os.environ.get('SORACOM_ENDPOINT',
                                   cfg.SORACOM_ENDPOINT)
...
class SoraCamClient(object):
    ...
    def _get(self, url: str, params: dict = {}) -> dict:
        ...
        """
        for i in range(_MAX_API_RETRIES):
            try:
    ...
    def _check_export_status(self, device_id: str, export_id: str,
        ...
        path = os.path.join(cfg.SORA_CAM_BASE_URL,
                            device_id, media, "exports", export_id)
        url = urljoin(self.api_endpoint, path)
        start_time = time.time()
        while (time.time() - start_time < cfg.WAITE_TIMEOUT):
            ...
            if status == expected:
                ...
                raise soracam.ExportFailedError(
                    ...
                    export {export_id}")
            time.sleep(cfg.LOOP_WAITE_TIME)
    ...
    def get_devices(self) -> dict:
        ...
        Raises:
            ...
            processing the response.
        """
        url = urljoin(self.api_endpoint, cfg.SORA_CAM_BASE_URL)
        return self._get(url)
    ...
    def get_devices_events(self, device_id: str = None, limit: int = 10,
        ...
        Raises:
            ...
            processing the response.
        """
        path = os.path.join(cfg.SORA_CAM_BASE_URL, 'events') \
            if not device_id else os.path.join(
            cfg.SORA_CAM_BASE_URL, device_id, 'events')
        url = urljoin(self.api_endpoint, path)
    ...
    def get_device(self, device_id=None) -> dict:
        ...
        device_path = os.path.join(cfg.SORA_CAM_BASE_URL, device_id)
